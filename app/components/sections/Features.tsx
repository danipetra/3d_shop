...
const Features = () => {
  return (
    <div id='features' className='max-w-5xl mx-auto pt-8'>
      ...
      <div className='flex flex-wrap items-center justify-center gap-8 ,ax-w-4xl mx-auto'>
        {features.map((features, index) => (
          <FeatureCard key={index} index={index} icon={features.icon} title={features.title} description={features.description}/>
        ))}
      </div>
    </div>
  )
