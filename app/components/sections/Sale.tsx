...
const Sale = () => {
  ...
  const isInView = useInView(mountRef, {once: true}); //animates the ref once it is in view, only once
  ...
  const rightModelFinalPosition : [number, number, number] = [2.9, 0.8, 0];

  const modelRotation : [number, number, number] = [
    Math.PI / 2, //90 degrees 
    Math.PI / 180 * 80, //80 degrees
    Math.PI / 180 * -10
  ];
